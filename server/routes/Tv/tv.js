...
router.get("/:id/like", (req, res) => {
  ...
  db.collection("user").findOne({ username: userId }, (err, result) => {
    ...
    if (result.tvLikes && result.tvLikes.includes(tvId)) {
...
router.post("/:id/like", (req, res) => {
  ...
  db.collection("user").findOne({ username: userId }, (err, user) => {
    ...
    // 이미 좋아요한 영화인 경우
    if (!user.tvLikes || !user.tvLikes.includes(tvId)) {
      ...
      db.collection("user").updateOne(
        ...
        (err) => {
          ...
          console.log(`${userId}가 ${tvId}를 좋아요하셨습니다.`);
      ...
      db.collection("user").updateOne(
        ...
        (err) => {
          ...
          console.log(`${userId}가 ${tvId}를 싫어요하셨습니다.`);
          return res.sendStatus(204);
        }
      );
    }
